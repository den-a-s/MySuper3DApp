...
void SceneState::onEnter() {
    ...
    SetWindowText(renderer.mDisplay->getHandlerWindow(), L"Scene");
}
...
void SceneState::handleInput(float deltaTime) {
    MSG msg = {};
...
void SceneState::update(float) {}

void SceneState::log(const std::string& msg) {
    mLog.push_back(msg);
    if (mLog.size() > 50) mLog.pop_front();
}

DirectX::XMFLOAT4 SceneState::getCameraPosition() const {
    auto& cam = mGame.getRenderer().getCamera();
...
void SceneState::render(float deltaTime) {
    auto& renderer = mGame.getRenderer();
